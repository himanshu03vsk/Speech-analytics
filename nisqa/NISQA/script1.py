...
args = {"mode": "predict_dir", "pretrained_model":"weights/nisqa.tar", # This argument list must be passed from the notebook.
        "data_dir":"recordings/", "output_dir":"results/", "ms_channel":None} 
...
def df_unify(df_nisqa, df_splitter, noi_thresh: float, save_locally=False):
    ...
    # print("iteration ", i)
        ...
        for sub_chunks in chunks.index:
            ...
            df_new = df_new.append({'filename': df_nisqa.loc[sub_chunks, "filename"],
             'duration': df_splitter.loc[i, "duration"],
             'confidence':concerned_params.loc["confidence"],'age':concerned_params.loc["age"],
             'gender': concerned_params.loc["gender"], 'accent': concerned_params.loc["accent"],
             'mos_pred': chunks.loc[sub_chunks, "mos_pred"], 'noi_pred':chunks.loc[sub_chunks, "noi_pred"],
             'dis_pred': chunks.loc[sub_chunks, "dis_pred"], 'col_pred':chunks.loc[sub_chunks, "col_pred"],
             'loud_pred':chunks.loc[sub_chunks, "loud_pred"], 'model':chunks.loc[sub_chunks, "model"], 
             'transcript':chunks.loc[sub_chunks, "transcript"]}, ignore_index=True)
